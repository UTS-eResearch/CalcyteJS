  ...
  return {
    ...
    template_html: function template_html(node, html) {
      if( node['@id'] === this.root_node['@id'] ) {
        return template_html_home(node, html);
      }
      var up_link;
      ...
      const json = JSON.stringify(this.helper.json_ld, null, 2);
    ...
    get_file_ref: function get_file_ref(dest_path, from_id) {
      ...
      if (this.multiple_files_dir && !this.is_root_node(from_id)) {
    ...
    dataset_to_html: function dataset_to_html(node) {
      ...
      console.log(`dataset_to_html ${node['@id']} / ${this.root_node['@id']} / ${this.first_page}`);
      if ( !this.is_root_node(node) && node["@label"] && !this.helper.get_name(node)) {
        ...
        console.log("Returning html early: " + html.slice(0, 10));
        return html;
      }


      console.log(`building HTML for ${node['@id']}`);
      ...
      if( this.first_page ) {
        console.log("setting first_page to false");
        this.first_page = false;
      }
      // Only shows in one-page mode. ???
      html += this.dataset_children_to_html(node);

      console.log(`Returning dataset_to_html ${node['@id']} - ${html.slice(0, 10)}`);
    ...
    init: function init(crate_data, out_path, multiple_files, template_path=defaults.catalog_template) {
      ...
      this.html_file_name = 'index.html'; // defaults.html_file_name;
      console.log(`index files = ${this.html_file_name}`);
      if (multiple_files) {
      ...
      // need to get the mtime of the Catalog.json outside the write_html loop
      //var stats = fs.statSync(catalog_actual_path);
      //var mtime = stats.mtime.toISOString();
      this.catalog_json_mtime = "TODO";
    ...
    make_index_html: function make_index_html(text_citation, zip_path) {
      ...
      for( var i in pages ) {
        ...
        console.log(`Writing to ${out_path}: "${page_html.slice(0, 10)}"`);
    ...
    make_index_pages: function make_index_pages(text_citation, zip_path) {
      ...
      for (let type of Object.keys(this.helper.item_by_type).sort()) {
        ...
        // the nodes created via the dataset children method above are not getting
        // passed back here

        for (let node of this.helper.item_by_type[type]) {
          if( this.multiple_files_dir ) {
            console.log(`node: ${node['@id']}`);
            pages.push({ node: node, html: this.dataset_to_html(node, true)});
          } else {
            body_el += this.dataset_to_html(node, true);
          }
        } 
      }
      ...
      console.log(`pages keys: ${pages.map((p) => p.node['@id'])}`);
