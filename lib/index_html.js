...
module.exports = function () {
  return {
    ...
    init_pure: function (options) {
      this.template = ejs.compile(options['template']);
      this.first_page = true;
      this.multiple_files = false;
    ...
    make_index_async: function make_index_async(text_citation, zip_path) {

      // async version: returns an array of { path, html }

      // Capturing the CATALOG.json as a string before the conversion loop
      // FIXME - this is still hard to test because it has a different @context than 
      // the source
